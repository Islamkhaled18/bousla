...
<main class="app sidebar-mini rtl">
    <div class="app-title">
        ...
        <ul class="app-breadcrumb breadcrumb side">
            ...
            @can('cities')
            <li class="breadcrumb-item active"><a href="{{ route('admin.cities.index') }}"
    ...
    @can('cities.create')
    <div>
        <a class="btn btn-primary btn-sm" href="{{ route('admin.cities.create') }}"
            title="انشاء مدينه جديده">انشاء مدينه جديده</a>
    </div>
    @endcan
                                ...
                                <td>
                                    ...
                                    <div class="form-check form-switch">
                                        <input class="form-check-input status-toggle" type="checkbox"
                                            id="status_{{ $city->id }}" data-id="{{ $city->id }}" {{
                                            $city->is_active ? 'checked' : '' }}>
                                        <label class="form-check-label" for="status_{{ $city->id }}">
                                            <span class="status-text">{{ $city->is_active ? 'نشط' : 'غير نشط'
                                                }}</span>
                                        </label>
                                    </div>
                                    @else
...
<script type="text/javascript">
    $('#sampleTable').DataTable();
     ...
     $('.status-toggle').change(function() {
        ...
        $.ajax({
            url: '{{ route("admin.cities.toggleStatus", ":id") }}'.replace(':id', categoryId),
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                is_active: isActive
