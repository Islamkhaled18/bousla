...
<main class="app sidebar-mini rtl">
    <div class="app-title">
        ...
        <ul class="app-breadcrumb breadcrumb side">
            ...
            @can('governorates')
            <li class="breadcrumb-item active"><a href="{{ route('admin.governorates.index') }}"
    ...
    <div>
        <a class="btn btn-primary btn-sm" href="{{ route('admin.governorates.create') }}"
            title="انشاء محافظه جديده">انشاء محافظه جديده</a>
    </div>
    @endcan
                                ...
                                <td>
                                    ...
                                    <div class="form-check form-switch">
                                        <input class="form-check-input status-toggle" type="checkbox"
                                            id="status_{{ $governorate->id }}" data-id="{{ $governorate->id }}" {{
                                            $governorate->is_active ? 'checked' : '' }}>
                                        <label class="form-check-label" for="status_{{ $governorate->id }}">
                                            <span class="status-text">{{ $governorate->is_active ? 'نشط' : 'غير نشط'
                                                }}</span>
                                        </label>
                                    </div>
                                    @else
                                    <span class="badge {{ $governorate->is_active ? 'bg-success' : 'bg-danger' }}">
...
<script type="text/javascript">
    $('#sampleTable').DataTable();
     ...
     $('.status-toggle').change(function() {
        ...
        $.ajax({
            url: '{{ route("admin.governorates.toggleStatus", ":id") }}'.replace(':id', categoryId),
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                is_active: isActive
